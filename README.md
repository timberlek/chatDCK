...
Si le code correspond à ce que le serveur attend alors la connection peut continuer sinon le serveur envoie "Clé incorrecte" et ferme la connexion.

Le client possède plusieur méthode personnelle:
...
- La méthode publique disconnect() qui ferme la connexion avec le serveur. Cette méthode est activée lorsque l'on ferme la fenetre. Cela perrmet d'eviter que le client reste connecté indéfiniment.
